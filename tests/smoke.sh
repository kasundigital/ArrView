...
docker exec "$APP" php -r '$p=new PDO("sqlite:/app/data/arrview.sqlite");$p->prepare("INSERT INTO app_settings(setting_key,setting_value) VALUES("ci_persistence","survives") ON CONFLICT(setting_key) DO UPDATE SET setting_value=excluded.setting_value")->execute();'
