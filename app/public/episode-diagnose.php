...
<?php if(!empty($result['queue'])):?><section class="panel"><h2>Download / import queue</h2><div class="release-list"><?php foreach($result['queue'] as $q):?><article class="release-row"><div><strong><?=e((string)$q['title'])?></strong><p class="meta"><?=e((string)$q['state'])?><?php if(!empty($q['download_client'])):?> · <?=e($q['download_client'])?><?php endif;?></p><?php if(!empty($q['output_path'])):?><p class="path-line"><?=e((string)$q['output_path'])?></p><?php endif;?></div><div class="release-status"><?=e((string)$q['state'])?></div><?php if(!empty($q['messages'])):?><div class="release-reasons"><?php foreach($q['messages'] as $m):?><span><?=e($m)?></span><?php endforeach;?></div><?php endif;?></article><?php endforeach;?></div></section><?php endif;?>
...
<?php if(!empty($result['history'])):?><section class="panel"><h2>Recent Sonarr history</h2><div class="history-list"><?php foreach(array_slice($result['history'],0,20) as $event):?><article class="history-row"><div class="history-type"><?=e($event['event_type']?:'event')?></div><div><strong><?=e($event['source_title']?:ucfirst((string)$event['event_type']))?></strong><p class="meta"><?=e(epDate($event['date']??null))?><?php if($event['quality']):?> · <?=e($event['quality'])?><?php endif;?><?php if($event['languages']):?> · <?=e($event['languages'])?><?php endif;?><?php if(!empty($event['download_client'])):?> · <?=e((string)$event['download_client'])?><?php endif;?></p><?php if($event['message']):?><p><?=e((string)$event['message'])?></p><?php endif;?><?php if(!empty($event['dropped_path'])):?><p class="path-line">Download path: <?=e((string)$event['dropped_path'])?></p><?php endif;?><?php if(!empty($event['imported_path'])):?><p class="path-line">Imported path: <?=e((string)$event['imported_path'])?></p><?php endif;?></div></article><?php endforeach;?></div></section><?php endif;?>
