...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    try {
        ...
        if ($action==='add') {
            ...
            $target=$pdo->prepare('SELECT role,enabled FROM users WHERE id=?');$target->execute([$id]);$target=$target->fetch();
            if($target && $target['role']==='admin' && (int)$target['enabled']===1 && $auth->enabledAdminCount()<=1) throw new RuntimeException('You cannot disable the final enabled administrator.');
            ...
            $target=$pdo->prepare('SELECT role,enabled FROM users WHERE id=?');$target->execute([$id]);$target=$target->fetch();
            if($target && $target['role']==='admin' && $role!=='admin' && (int)$target['enabled']===1 && $auth->enabledAdminCount()<=1) throw new RuntimeException('You cannot demote the final enabled administrator.');
            ...
            $target=$pdo->prepare('SELECT role,enabled FROM users WHERE id=?');$target->execute([$id]);$target=$target->fetch();
            if($target && $target['role']==='admin' && (int)$target['enabled']===1 && $auth->enabledAdminCount()<=1) throw new RuntimeException('You cannot delete the final enabled administrator.');
...
<header class="topbar"><a class="brand" href="/">ArrView <small class="version-chip">v<?=e(ARRVIEW_VERSION)?></small></a><nav><a href="/">Library</a><a href="/admin.php">Instances</a><a class="active" href="/users.php">Users</a><a href="/system.php">System</a><a href="/support.php">Support</a><a href="/logout.php">Logout</a></nav></header>
...
<section class="panel"><h2>Add User</h2><form method="post" class="instance-form"><?=csrf_field()?> <input type="hidden" name="action" value="add"><label>Username<input name="username" minlength="3" required></label><label>Role<select name="role"><option value="viewer">Viewer</option><option value="admin">Admin</option></select></label><label>Password<input name="password" type="password" minlength="8" required></label><label>Confirm<input name="confirm_password" type="password" minlength="8" required></label><button class="primary">Create User</button></form></section>
...
<form method="post"><?=csrf_field()?> <input type="hidden" name="action" value="role"><input type="hidden" name="id" value="<?=$u['id']?>"><select name="role" onchange="this.form.submit()"><option value="viewer" <?=$u['role']==='viewer'?'selected':''?>>Viewer</option><option value="admin" <?=$u['role']==='admin'?'selected':''?>>Admin</option></select></form>
<form method="post"><?=csrf_field()?> <input type="hidden" name="action" value="toggle"><input type="hidden" name="id" value="<?=$u['id']?>"><button><?=$u['enabled']?'Disable':'Enable'?></button></form>
<form method="post" class="inline-reset"><?=csrf_field()?> <input type="hidden" name="action" value="password"><input type="hidden" name="id" value="<?=$u['id']?>"><input name="password" type="password" minlength="8" placeholder="New password" required><button>Reset</button></form>
<form method="post" onsubmit="return confirm('Delete this user?')"><?=csrf_field()?> <input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?=$u['id']?>"><button class="danger">Delete</button></form>
