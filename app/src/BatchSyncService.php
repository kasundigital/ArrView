...
SQL;
        ...
        try {
            foreach ($this->streamArray($instance, '/api/v3/movie') as $movie) {
...
SQL;
        ...
        $this->pdo->beginTransaction();
        try {
            foreach ($this->streamArray($instance, '/api/v3/series') as $series) {
    ...
    private function streamArray(array $instance, string $path): Generator
    {
        $tmp = tempnam(sys_get_temp_dir(), 'arrview_');
        if ($tmp === false) {
            throw new RuntimeException('Could not create temporary sync file.');
        }

        $fp = fopen($tmp, 'w+b');
        if ($fp === false) {
            @unlink($tmp);
            throw new RuntimeException('Could not open temporary sync file.');
        }
        ...
        try {
            ...
            while (!feof($fp)) {
                ...
                if ($chunk === false) {
                    throw new RuntimeException('Failed reading streamed Arr response.');
                }
                $len = strlen($chunk);
                for ($i = 0; $i < $len; $i++) {
                    $c = $chunk[$i];
    ...
    {
        if (!$seen) return;
        foreach (array_chunk($seen, 900) as $chunk) {
            // Keep chunking compatible with SQLite parameter limits. Actual stale cleanup is done after all IDs are known.
        }
